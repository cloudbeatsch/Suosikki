...
namespace Suosikki.WebJobs.CognitiveServices
{
    public class RecommendationsApiWrapper
    {
        ...
        private Dictionary<string, ModelInfo> modelInfoDic = new Dictionary<string, ModelInfo>();

        public RecommendationsApiWrapper(string accountKey)
        {
        ...
        {
            if (!modelInfoDic.ContainsKey(modelType))
            {
                // retrieve or create the models
                modelInfoDic[modelType] = GetModelByName(modelType);
                if (modelInfoDic[modelType] == null)
                {
                    modelInfoDic[modelType] = CreateModel(modelType, modelType);
                }
            }
            return modelInfoDic[modelType];
        }

        public ModelInfo GetModelByName(string modelName)
        {
            var queryString = HttpUtility.ParseQueryString(string.Empty);
            var uri = "https://westus.api.cognitive.microsoft.com/recommendations/v4.0/models?" + queryString;

            // get all models
            var response = httpClient.GetAsync(uri).Result;
            string jsonString = ExtractReponse(response);

            var a = new { models = new ModelInfo[] { } };
            dynamic obj = JsonConvert.DeserializeObject(jsonString, a.GetType());

            // delete all models
            foreach (ModelInfo model in obj.models)
            {
        ...
        {
            Console.WriteLine("Deleting all existing models");  
            var queryString = HttpUtility.ParseQueryString(string.Empty);
            var uri = "https://westus.api.cognitive.microsoft.com/recommendations/v4.0/models?" + queryString;
        ...
        public ModelInfo CreateModel(string modelName, string description = null)
        {
            ...
            var response = httpClient.PostAsJsonAsync(uri, modelRequestInfo).Result;
        ...
        public CatalogImportStats UploadCatalog(string modelType, string filePath, string fileName)
        {
            Console.WriteLine("Uploading Catalog {0}", fileName);
            var model = GetModel(GetModelInfo(modelType).Id);
            return UploadCatalogInternal(model.Id, filePath, fileName);
        }

        private CatalogImportStats UploadCatalogInternal(string modelId, string filePath, string fileName)
        {
            string uri = BaseUri + "/models/" + modelId + "/catalog?catalogDisplayName=" + fileName;
            using (var filestream = new FileStream(filePath, FileMode.Open, FileAccess.Read))
            {
                var response = httpClient.PostAsync(uri, new StreamContent(filestream)).Result;
        ...
        public CatalogImportStats UploadUsage(string modelType, string filePath, string fileName)
        {
            Console.WriteLine("Uploading Usage {0}", fileName);
            var model = GetModelInfo(modelType);
            return UploadUsageInternal(model.Id, filePath, fileName);
        }

        public CatalogImportStats UploadUsageInternal(string modelId, string filePath, string fileName)
        {
            string uri = BaseUri + "/models/" + modelId + "/usage?usageDisplayName=" + fileName;
            using (var filestream = new FileStream(filePath, FileMode.Open, FileAccess.Read))
            {
                var response = httpClient.PostAsync(uri, new StreamContent(filestream)).Result;
                ...
                {
                    throw new Exception(
                        String.Format("Error {0}: Failed to import usage items {1}, for model {2} \n reason {3}",
                            response.StatusCode, filePath, modelId, ExtractErrorInfo(response)));
                }
        ...
        public long BuildModel(string modelId, BuildRequestInfo buildRequestInfo, out string operationLocationHeader)
        {
            ...
            var response = httpClient.PostAsJsonAsync(uri, buildRequestInfo).Result;
        ...
        public long BuildRecommondationModel(string modelType, string description)
        {
            string olHeader;
            return CreateRecommendationsBuild(GetModelInfo(modelType).Id, description, true, out olHeader);
        }
        /// <summary>
        ...
        public ModelInfo GetModel(string modelId)
        {
            ...
            {
                throw new Exception(
                    String.Format("Error {0}: Failed to get modelId {1}, \n reason {2}",
                        response.StatusCode, modelId, ExtractErrorInfo(response)));
            }
            var jsonString = ExtractReponse(response);
            var modelInfo = JsonConvert.DeserializeObject<ModelInfo>(jsonString);
        ...
        public long CreateRecommendationsBuild(string modelId, string buildDescription,
        ...
        public OperationInfo<T> WaitForOperationCompletion<T>(string operationId)
        {
            ...
            while (true)
            {
                var response = httpClient.GetAsync(uri + "/" + operationId).Result;
                var jsonString = response.Content.ReadAsStringAsync().Result;
        ...
        public string StartBatchJob(BatchJobsRequestInfo batchJobsRequestInfo, out string operationLocationHeader)
        {
            ...
            var response = httpClient.PostAsJsonAsync(uri, batchJobsRequestInfo).Result;
            var jsonString = response.Content.ReadAsStringAsync().Result;
